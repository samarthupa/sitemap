...
# Function to check if the request is coming from allowed domain
def check_domain():
    if request.referrer:
        referrer_domain = request.referrer.split('/')[2]
        if referrer_domain == 'samarthupadhyay.com':
            return True
    return False
...
def index():
    ...
    return "Access Denied: Embedding is only allowed from samarthupadhyay.com"
