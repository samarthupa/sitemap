    ...
    try:
        ...
        if status_code in [301, 307, 302]:
            while 'location' in response.headers:
                ...
                redirection_status_codes.append(response.status_code)
...
if st.button("Submit"):
    st.text("Analyzing URLs. It may take time if there are many URLs...")
    with st.spinner():
        ua = UserAgent()
        selected_user_agent = ""
        ...
        # Create Excel file with one sheet
        excel_data = {'Main Sheet': main_data}
        excel_file = BytesIO()
